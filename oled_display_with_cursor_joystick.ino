...
void setup()   {                
 ...
 // display.display(); //display adafruit buffer icon 
  delay(2000);
...
void movingbitmap(const uint8_t *bitmap, uint8_t w, uint8_t h) {
 ...
 // icons[YPOS] = height/2;
  ...
  while (1) {
    ...
    joystick_y = 1023-analogRead(ANALOG_Y);
    joystick_z = digitalRead(ANALOG_Z);
    icons[XPOS] = (joystick_x/1023.0)*width + XA;
    icons[YPOS] = (joystick_y/1023.0)*height + YA;
    ...
    if(digitalRead(ANALOG_Z)){
      if(cond1)
      {
        cond1 = false;
        XA  = icons[XPOS] - width/2;
        YA = icons[YPOS] - height/2;
      }
    }
    else
    {
